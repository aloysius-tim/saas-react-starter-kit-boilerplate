      ...
      <v-card class="elevation-12">
        ...
        <v-card-text>
          <v-form>
            ...
            <v-text-field prepend-icon="lock" name="new-password" v-model="newPassword" label="New Password" type="password"></v-text-field>
          </v-form>
        </v-card-text>
        <v-card-actions>
...
export default {
  ...
  methods: {
    doSubmit () {
      this.busy = true
      this.$store.dispatch('auth/updatePassword', { password: this.password, newPassword: this.newPassword }).then(() => {
        this.busy = false
        redirectIntended()
      }).catch(() => {
        this.busy = false
